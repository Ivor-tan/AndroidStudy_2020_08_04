 ...
 * 即对于给定的n个物品，设v[i],w[i]分别为第i个物品的价值和重量，C为背包的容量。每件物品不能重复装入背包
...
// v[i]->v[i-1]替换为取已放入当前物品后剩余空间的最大价值（前提改为商品可重复）
// v[i][j - w[i - 1]]意为放了这个物品之后再减去当前物品的重量的最大价值
//                    if (v[i - 1][j] < val[i - 1] + v[i][j - w[i - 1]]) {
